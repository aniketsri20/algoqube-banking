...
const cli = await y
    ...
    .coerce('input', (input) => normalizePaths(input, parsed.patterns))
    .option('output', {
    alias: 'o',
    ...
    .coerce('output', (output) => normalizePaths(output, parsed.patterns))
    .option('format', {
    alias: 'f',
    describe: 'Format',
...
const parsers = [new PipeParser(), new DirectiveParser(), new ServiceParser()];
if (cli.marker) {
...
if (cli.stripPrefix) {
    postProcessors.push(new StripPrefixPostProcessor({ prefix: cli.stripPrefix }));
}
if (cli.sort) {
